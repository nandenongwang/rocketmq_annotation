...
public class Broker2Client {
    ...
    /**
     * 通知消费者重置消费进度到指定时间戳
     * force -> 强制指定、无视指定进度大于当前消费进度
     * timeStamp -> 指定到该时间 -1则指定到最大offset
     */
        ...
        for (Map.Entry<Channel, ClientChannelInfo> entry : channelInfoTable.entrySet()) {
            ...
            //region 向消费者发送命令 获取到各消费者消费进度
                ...
                try {
                    ...
                    log.error("[get-consumer-status] get consumer status exception. topic={}, group={}, error={}",
                            topic, group, e.toString());
                }
