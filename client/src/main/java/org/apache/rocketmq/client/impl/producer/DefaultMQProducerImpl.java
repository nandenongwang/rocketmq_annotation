...
public class DefaultMQProducerImpl implements MQProducerInner {
     ...
     * @param currentLatency 延时级别 一般取值调用耗时
     * @param isolation      无视延时级别 默认不可用延时30s
     */
    ...
    private SendResult sendDefaultImpl(Message msg, CommunicationMode communicationMode, SendCallback sendCallback, long timeout)
        ...
        if (topicPublishInfo != null && topicPublishInfo.ok()) {
            ...
            //region 多次尝试均发送失败
            //设置异常信息 发送超时、连接broker超时、访问broker超时、broker不存在、其他异常等
            ...
            if (callTimeout) {
                //会把前面错误吞掉
                if (exception != null) {
                    log.error("timeout cause", exception);
                }
    ...
    private SendResult sendKernelImpl(Message msg,
        ...
        if (brokerAddr != null) {
            ...
            //region 是否通过vip通道发送 vip通道端口 = 正常端口 - 2 【server端口】
            ...
            try {
                ...
                //region hook前置方法调用
                //region 执行CheckForbiddenHook 默认无
                ...
                //region 执行snedMessageHook before tracemessage使用
                ...
                switch (communicationMode) {
                    case ASYNC:
                        //region clone消息【防止重复压缩】 & 设置namespace
                ...
                //region 正常执行snedMessageHook after tracemessage使用
                if (this.hasSendMessageHook()) {
            ...
            //region 异常执行snedMessageHook after 然后抛出异常
            catch (RemotingException e) {
                if (this.hasSendMessageHook()) {
